    ...
    <nav className="w-full h-auto fixed font-montserrat border-b border-gray-200 bg-white z-50 pt-3">
      <div className="flex flex-col space-y-5 ">
        <div className="relative flex items-center p-4 md:px-16">
          ...
          <Link to="/" className="absolute left-1/2 transform -translate-x-1/2 whitespace-nowrap">
        ...
        <div className='flex justify-around items-center w-full'>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {sublistContent[link]?.map((subcategory, index) => (
                              <Link
                                key={index}
                                to={`/${link.toLowerCase().replace(/\s+/g, '-')}/${subcategory.toLowerCase().replace(/\s+/g, '-')}`}
          ...
          <div className='hidden md:flex p-3 text-md border rounded-lg'> <SearchFilter  /></div>
        ...
        <div className={`${menuOpen ? "block" : "hidden"} md:hidden px-8 p-6`}>
          ...
          <div className='block md:hidden my-3'><SearchInput /></div>
          <div className='block md:hidden p-3 text-md border rounded-lg'> <SearchFilter  /></div>
        </div>
      </div>
    </nav>
  );
