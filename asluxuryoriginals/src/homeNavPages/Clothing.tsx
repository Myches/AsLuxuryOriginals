      ...
      <div className="flex items-start justify-between relative">
        <div className="relative mb-12 flex-[0.5] md:flex-[0.75]">
          ...
          <ul
            ...
            } absolute top-full left-0 w-full bg-white z-50 shadow-lg border-t-2 border-gray-300 md:relative md:top-auto md:w-auto md:bg-transparent md:shadow-none md:block md:flex md:items-center md:flex-wrap md:gap-y-2 md:mt-0 md:border-t-0 z-0`}
          >
            {clothingFilters.map((filter: string, index: number) => (
              <li
                key={index}
                className="border-b-2 md:border-2 md:rounded-lg px-5 py-2 mr-0 md:mr-2.5"
              >
                <Link to="">{filter}</Link>
              </li>
            ))}
